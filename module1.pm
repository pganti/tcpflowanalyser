...
sub main {
	...
	if( !($pt eq 'ack') and ($SN =~ /(\d+)\:(\d+)/) and !($type eq 'S') and ($bytes > 0) ) {
...
#-- group components
    ...
    sub printGroup {
	my $longueur1 = scalar @groupPacketList;
	my $longueur2 = scalar @dupOrFill;
	my $longueur = &max( $longueur1 , $longueur2 );
	for( $i=0 ; $i<$longueur ; $i++ ) {
	    print "group $i: ";
	    my $l = scalar @{ $groupPacketList[$i] };
	    for( $j=0; $j<$l ; $j++ ) {
		print $groupPacketList[$i][$j] , " | ";
	    }
	    print "\n";
	    print "group $i: ";
	    $l = scalar @{ $dupOrFill[$i] };
	    for( $j=0; $j<$l ; $j++ ) {
		print $dupOrFill[$i][$j] , "|";
	    }
	    print "\n";
...
#--- find group of which packet is a dup or filler
sub findGroup {
    local @table = @maxSN;
    if( ($newGroup == 0) and ( defined $maxSN[$currentGroup] ) ) {
	pop @table;
...
#--- make the difference between two timestamps and converts into microseconds
sub substractTS {
    $t1 = shift;
    $t2 = shift;
